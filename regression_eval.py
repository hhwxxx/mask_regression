...
flags.DEFINE_integer('eval_interval_secs', 60 * 2, 'Evaluation interval seconds.')


def eval(model_variant, dataset_split):
    with tf.Graph().as_default() as g:
        with tf.device('/cpu:0'):
            images, labels = input_pipeline.inputs(
                dataset_split, FLAGS.is_training, 
        ...
        num_batches = int(math.ceil(input_pipeline.NUMBER_VAL_DATA / FLAGS.batch_size))
...
def main(unused_argv):
    while True:
        eval(FLAGS.model_variant, FLAGS.dataset_split)
